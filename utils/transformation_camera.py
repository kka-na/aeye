...
tici_e_focal_length = tici_d_focal_length = 567.0
...
def get_view_frame_from_road_frame(roll, pitch, yaw, height):
    device_from_road = orient.rot_from_euler(
        [roll, pitch, yaw]).dot(np.diag([1, -1, -1]))
...
def denormalize(img_pts, intrinsics=fcam_intrinsics, width=np.inf, height=np.inf):
    ...
    img_pts = np.atleast_2d(img_pts)
    img_pts = np.hstack(
        (img_pts, np.ones((img_pts.shape[0], 1), dtype=img_pts.dtype)))
...
def get_camera_frame_from_calib_frame(camera_frame_from_road_frame, intrinsics=fcam_intrinsics):
    ...
    camera_frame_from_calib_frame = np.dot(
        camera_frame_from_ground, ground_from_calib_frame)
