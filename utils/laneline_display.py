...
import os
from ui_helpers import _BB_TO_FULL_FRAME
...
vipc_client = VisionIpcClient(
    "camerad", VisionStreamType.VISION_STREAM_RGB_ROAD, True)
...
while 1:
    ...
    imgff = np.frombuffer(rgb_img_raw, dtype=np.uint8).reshape(
        (vipc_client.height, vipc_client.width, 3))
    imgff = imgff[:, :, ::-1]  # Convert BGR to RGB

    zoom_matrix = _BB_TO_FULL_FRAME[num_px]
    cv2.warpAffine(imgff, zoom_matrix[:2], (img.shape[1],
                   img.shape[0]), dst=img, flags=cv2.WARP_INVERSE_MAP)
