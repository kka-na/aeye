...
class BSD:
    ...
    def bsdPublisher(self):
        ...
        for box in self.car_array:
            ...
            if any(-15 < x for x in car_x_array) & any(15 > x for x in car_x_array) & any(2 < y for y in car_y_array) & any(5 > y for y in car_y_array):
                bsd_msg.data[0] = 1
            else:
                bsd_msg.data[0] = 0

            if any(-15 < x for x in car_x_array) & any(15 > x for x in car_x_array) & any(-5 < y for y in car_y_array) & any(-2 > y for y in car_y_array):
                bsd_msg.data[1] = 1
            else:
                bsd_msg.data[1] = 0
